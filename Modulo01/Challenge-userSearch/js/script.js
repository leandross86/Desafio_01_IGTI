...
function renderStatistics(users) {
  ...
  panelStatistics.innerHTML = `
      ...
      <ul fixed>
        <li class="card-stat">Sexo masculino: <strong>${countMale}</strong> </li>
        <li class="card-stat">Sexo feminino: <strong>${countFemale}</strong></li>
        <li class="card-stat">Soma das idades: <strong>${formatNumber(sumAges)}</strong></li>
        <li class="card-stat">Média das idades: <strong>${formatAverage(averageAges)}</strong></li>
      </ul>
    `;
}
